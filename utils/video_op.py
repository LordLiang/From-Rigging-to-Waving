...
'''@torch.no_grad()
def save_video_refimg_and_pose(
    local_path,
    ref_frame,
    gen_video, 
    dwpose, 
    ...
    # text_images = text_images.unsqueeze(1) # Tensor 8x1x256x256x3
    # text_images = text_images.repeat_interleave(repeats=gen_video.size(2), dim=1) # 8x16x256x256x3
    ref_frame = ref_frame.unsqueeze(2).repeat_interleave(repeats=gen_video.size(2), dim=2) # b c f h w
    ref_frame = ref_frame.mul_(vid_std).add_(vid_mean)
    # ref_frame = ref_frame.repeat_interleave(repeats=gen_video.size(2), dim=2) # 8x16x256x256x3
    ref_frame.clamp_(0, 1)
    ref_frame = ref_frame * 255.0
    ...
    dwpose = rearrange(dwpose, 'b c f h w -> b f h w c')
    dwpose.clamp_(0, 1)
    dwpose = dwpose * 255.0
    dwpose_ref = dwpose[:,:1].repeat_interleave(repeats=gen_video.size(2), dim=1)
    dwpose = dwpose[:,1:]

    images = rearrange(gen_video, 'b c f h w -> b f h w c')
    images = torch.cat([ref_frame, dwpose_ref, images, dwpose], dim=3)
...
def save_video_refimg_and_pose(
    ...
    if local_image is not None:
        local_image = rearrange(local_image, 'b c f h w -> b f h w c')
        local_image.clamp_(0, 1)
        local_image = local_image * 255.0
        images = torch.cat([images, local_image], dim=3)
    ...
    for _ in [None] * retry:
        try:
            ...
            if len(images) == 1:
                ...
                outputs = []
                local_path_dir = local_path.split('.')[0]
                os.makedirs(local_path_dir, exist_ok=True)
                for i, image_name in enumerate(images):
                    x = Image.fromarray(image_name)
                    x.save(os.path.join(local_path_dir, str(i).zfill(4)+'.png'))
                    outputs.append(x)
