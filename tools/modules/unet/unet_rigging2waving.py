...
class UNetSD_Rigging2Waving(nn.Module):
    ...
    def __init__(self,
        ...
        super(UNetSD_Rigging2Waving, self).__init__()
        self.zero_y = zero_y
        ...
        if 'coarse_image' in self.video_compositions:
            if latent_coarse_image:
                self.coarse_image_embedding = nn.Sequential(
                    nn.Conv2d(5, concat_dim * 4, 3, padding=1),
                    nn.SiLU(),
            ...
            self.coarse_image_embedding_after = zero_module(Transformer_v2(heads=2, dim=concat_dim, dim_head_k=concat_dim, dim_head_v=concat_dim, dropout_atte = 0.05, mlp_dim=concat_dim, dropout_ffn = 0.05, depth=adapter_transformer_layers))
    ...
    def forward(self, 
        ...
        concat = x.new_zeros(batch, self.concat_dim, f, h, w) # b 8 f h w
                    
        # coarse_image_embedding (coarse frames)
        if coarse_image is not None: # b c f h w -> c=4,4,1?
            coarse_image = rearrange(coarse_image, 'b c f h w -> (b f) c h w')
            coarse_image = self.coarse_image_embedding(coarse_image)

            h = coarse_image.shape[2]
            coarse_image = self.coarse_image_embedding_after(rearrange(coarse_image, '(b f) c h w -> (b h w) f c', b = batch))
            coarse_image = rearrange(coarse_image, '(b h w) f c -> b c f h w', b = batch, h = h)

            concat = concat + misc_dropout(coarse_image)
        ...
        if dwpose is not None:
            ...
            h = dwpose.shape[2] # torch.Size([1, 8, 96, 64]), 
