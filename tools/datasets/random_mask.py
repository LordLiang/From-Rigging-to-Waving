...
def create_random_shape_with_random_motion(
    ...
    foreground=None,
    org_masks=None,
    ensure_all_frames_in_foreground=True,  
    turn_background_mask=True,
    weights=[1, 1, 2],
):
    if turn_background_mask:
        fill_value = 255
    else:
        fill_value = 0
    ...
    foreground[0] = np.array(foreground[0])
    if foreground[0] is not None and ensure_all_frames_in_foreground:
        foreground_indices = np.argwhere(foreground[0])
        if foreground_indices.size > 0:
            valid_positions = []
            for i, j in foreground_indices:
                if i <= imageHeight - region_height and j <= imageWidth - region_width:
            ...
            if valid_positions:
                ...
                print("Warning: No valid foreground position for the mask size. Falling back to random position.")
        else:
            print("Warning: No foreground pixels found. Falling back to random position.")

    velocity = get_random_velocity(max_speed=random.uniform(3, 5))
    m = Image.fromarray(np.zeros((imageHeight, imageWidth)).astype(np.uint8))
    
    mask_type = random.choices(['square', 'org', 'random'], weights=weights)[0]
    
    if mask_type == 'org' and (org_masks is not None):
        # erode or dilate the mask
        if random.random() > 0.5:
            kernel_size = random.randint(5, 15)
            kernel = np.ones((kernel_size, kernel_size), np.uint8)
        #if random.random() > 0.5:
        #    augmented_masks = [cv2.erode(np.array(mask), kernel, iterations=1) for mask in org_masks]
        #else:
            org_masks = [cv2.dilate(np.array(mask), kernel, iterations=1) for mask in org_masks]
            print('using org mask')
            org_masks = [Image.fromarray(mask.astype(np.uint8)) for mask in org_masks]
        return org_masks, fill_value
    
    else:
        
        if mask_type == 'square':
            region = Image.new('L', (region_width, region_height), 255)
        
        m.paste(region, (y, x, y + region_width, x + region_height))
        mask_np = np.array(m).astype(np.uint8) 


        if foreground is not None and turn_background_mask:
            background_mask = (foreground[0] == 0).astype(np.uint8)  
            m = np.logical_or(mask_np, background_mask).astype(np.uint8)  
        ...
        if random.uniform(0, 1) > 0.5:
            if foreground is not None and turn_background_mask:
                for i in range(video_length - 1):
                    foreground[i+1] = np.array(foreground[i+1])
                    background_mask = (foreground[i+1] == 0).astype(np.uint8)
                    m = np.logical_or(mask_np, background_mask).astype(np.uint8)
                    m= Image.fromarray(m * 255)
                    masks.append(m.convert('L'))
                return masks, fill_value
            else:
                return masks * video_length, fill_value
            ...
            for i in range(video_length - 1):
                foreground[i+1] = np.array(foreground[i+1])
                for _ in range(10):  
                    new_x, new_y, velocity = random_move_control_points(
                        new_x, new_y, imageHeight, imageWidth, velocity, (region_height, region_width),
                        maxLineAcceleration=(3, 0.5), maxInitSpeed=3
                    )
                    temp_mask = np.zeros((imageHeight, imageWidth), dtype=np.uint8)
                    temp_mask[new_x:new_x + region_height, new_y:new_y + region_width] = 1
                    overlap = np.logical_and(temp_mask, foreground[i+1])
                    if np.any(overlap):
                        x, y = new_x, new_y
                        break
                else:
                    print("Warning: Cannot find a valid position in foreground. Keeping previous position.")
                ...
                if foreground is not None and turn_background_mask:
                    ...
                    background_mask = (foreground[i+1] == 0).astype(np.uint8)
                    m = np.logical_or(m, background_mask).astype(np.uint8)
                    m = Image.fromarray(m * 255)
