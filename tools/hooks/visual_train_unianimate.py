...
class VisualTrainImageToVideo(object):
    ...
    def run(self,
        ...
        #print(cfg.ddim_timesteps)
        #print(self.guide_scale)
        ...
        for keys in self.partial_keys_list:
            ...
            dwpose = dwpose[:viz_num]
            if local_image is not None:
                local_image = local_image[:viz_num].cpu()
            ...
            print('start_save')
            save_video_refimg_and_pose(local_path, ref_frame.cpu(), video_data.cpu(),  dwpose.cpu(), local_image=local_image, mean=cfg.mean, std=cfg.std, text_size=text_size)
            '''try:
                save_video_refimg_and_pose(local_path, ref_frame.cpu(), video_data.cpu(),  dwpose.cpu(), local_image=local_image, mean=cfg.mean, std=cfg.std, text_size=text_size)
