...
def inference_rigging2waving_entrance(cfg_update,  **kwargs):
...
def load_video_frames(ref_image_path, pose_file_path, train_trans, vit_transforms, train_trans_pose, train_trans_mask,max_frames=32, frame_interval = 1, resolution=[512, 768], get_first_frame=True, vit_resolution=[224, 224], kernel_size=15, iters=10, mask_dilation=False):
    ...
    for ii_index in sorted(os.listdir(pose_file_path)):
        if ii_index != "ref_dwpose.png":
            ...
            if os.path.exists(pose_file_path.replace('openpose', 'mask')):
                ...
                '''
                set the square mask by (top_left, bottom_right, top_left, bottom_right)
                mask = np.zeros((resolution[1], resolution[0])).astype(np.uint8)
                mask[top_left:bottom_right, top_left:bottom_right] = 255
                mask_all[ii_index] = Image.fromarray(mask)
                '''
    ...
    if frame_list:
        ...
        return vit_frame, video_data, misc_data, dwpose_data, coarse_data, random_ref_frame_data, random_ref_dwpose_data, mask_data
...
def worker(gpu, cfg, cfg_update):
    ...
    for idx, file_path in enumerate(test_list):
        ...
        vit_frame, _, misc_data, dwpose_data, coarse_data, random_ref_frame_data, random_ref_dwpose_data, mask_data = load_video_frames(ref_image_key, 
        ...
        vit_frame_nc, _, misc_data_nc, _, _, random_ref_frame_data_nc, _, _ = load_video_frames(ref_image_key.replace('texture', 'ffc_resnet_inpainted'), 
                                                                                                                                                                   pose_seq_key, train_trans, vit_transforms, train_trans_pose, train_trans_mask, max_frames=cfg.max_frames, frame_interval =cfg.frame_interval, resolution=cfg.resolution, mask_dilation=cfg.mask_dilation, kernel_size=cfg.kernel_size)
        ...
        with amp.autocast(enabled=True):
            ...
            for partial_keys_one in partial_keys:
                ...
                video_data = diffusion.ddim_sample_blend_loop(
                    noise=noise_one,
